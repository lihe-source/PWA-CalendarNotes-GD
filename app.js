...
function dateTime24Html(prefix,label,date,opts={}){
  ...
  return `<fieldset class="datetime24-group" data-prefix="${attr(prefix)}"><legend>${esc(label)} <span class="format-24h">24H</span></legend><div class="datetime24-row"><label class="datetime24-date"><span>日期</span><input id="${attr(prefix)}Date" type="date" value="${valid?localDateValue(date):''}" ${allowEmpty?'':'required'}></label><div class="datetime24-time-row" role="group" aria-label="${esc(label)} 時間"><label class="datetime24-hour"><span>時</span><select id="${attr(prefix)}Hour" aria-label="${esc(label)} 小時">${timeOptionHtml(23,h)}</select></label><span class="datetime24-colon">:</span><label class="datetime24-minute"><span>分</span><select id="${attr(prefix)}Minute" aria-label="${esc(label)} 分鐘">${timeOptionHtml(59,m)}</select></label></div></div></fieldset>`;
}
