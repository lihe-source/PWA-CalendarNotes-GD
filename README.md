...
V2.1.0 新增背景自動登入：完成一次 Google 授權後，App 再次開啟會先顯示本機資料，再由 Worker 驗證保存的會話、更新 Google access token、連接原共享工作區並同步，全程不需要再按登入。網路中斷時保留會話與帳號資料，恢復連線或回到 App 時自動重試。

設定頁會顯示「自動登入已啟用」、會話閒置期限或缺少的部署設定。自動登入必須設定 `GOOGLE_CLIENT_SECRET` 與 `AUTH_ENCRYPTION_KEY`；首次授權、新裝置、清除網站資料、Google 撤銷權限或最長會話到期時，Google 仍會要求使用者授權。
...
5. 開啟後先呈現本機內容，背景恢復連線。Worker 會驗證保存的應用程式會話並更新 Google 權杖，完成首次授權後再次開啟不需要再按登入。
